...
class BundleParser {
    ...
    static getOperation(entry) {
        const { operation } = fhir_works_on_aws_interface_1.getRequestInformation(entry.request.method, entry.request.url);
        if (operation === 'vread') {
     ...
     * return BatchReadWriteRequests that can be executed to write the Bundle entries to the Database
     */
    static async updateReferenceRequestsIfNecessary(requests, dataService, serverUrl) {
        ...
        return this.checkReferences(orderedBundleEntriesId, idToRequestWithRef, fullUrlToRequest, allRequests, serverUrl, dataService);
    }
    /**
     ...
     * @return BatchReadWriteRequests that can be executed to write the Bundle entries to the Database
     */
    static async checkReferences(orderedBundleEntriesId, idToRequestWithRef, fullUrlToRequest, allRequests, serverUrl, dataService) {
            ...
            if (requestWithRef.references) {
                for (let j = 0; j < requestWithRef.references.length; j += 1) {
                    ...
                    if (reference.referenceFullUrl in fullUrlToRequest) {
                        ...
                        set_1.default(requestWithRef, `resource.${reference.referencePath}`, `${reqBeingReferenced.resourceType}/${id}`);
                    ...
                    if (!referenceIsFound && [serverUrl, `${serverUrl}/`].includes(reference.rootUrl)) {
                        ...
                        set_1.default(requestWithRef, `resource.${reference.referencePath}`, `${requestWithRef.resourceType}/${reference.id}`);
                        referenceIsFound = true;
                    }
                    if (!referenceIsFound) {
    ...
    static getReferences(entry) {
        const flattenResource = flat_1.default(get_1.default(entry, 'resource', {}));
        const referencePaths = Object.keys(flattenResource).filter(key => key.includes('reference'));
        if (referencePaths.length === 0) {
            return [];
        }
        const references = referencePaths.map(referencePath => {
            const entryReference = get_1.default(entry.resource, referencePath);
            ...
            if (fullUrlMatch) {
                ...
                if (rootUrl === undefined) {
                    if (entry.fullUrl.length > 0 && entry.fullUrl.match(regExpressions_1.captureFullUrlParts)) {
